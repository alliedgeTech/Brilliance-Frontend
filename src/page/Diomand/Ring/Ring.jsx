...
function Shop() {
  ...
  const [ringData, setRings] = useState(
    location.state && location.state.ringData
  );
  ...
  const fetchData = async () => {
    try {
      ...
      const selectedColorLabels = selectedColors
      ...
      const response = await axios.get(
        `http://localhost:6001/api/v1/getRing?page=${page}&minPrice=${minPrice}&maxPrice=${maxPrice}&colors=${selectedColorLabels}&catogary=${selectedCategories.join(
          ","
        )}`
      );
      setDiamonds((prevDiamonds) =>
        page === 1 ? response.data : [...prevDiamonds, ...response.data]
      );
    } catch (error) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          "http://localhost:6001/api/v1/AdminDash/getRingCatogary"
        );
                              ...
                              {colors.map((color) => (
                                ...
                                >
                                  <input
                                    ...
                                    readOnly
                                    className={`form-checkbox h-4 w-4 text-${
                                      color.color.split("-")[1]
                                    }-500 border-gray-300 rounded mr-2`}
                                  />
                            ...
                            {Diomands.map((diamond, index) => (
                              <div key={index} className="col-span-1">
                                <div className="products-entry clearfix product-wapper shadow-md h-full">
                                  <Link
                                    to={{
                                      pathname: `/singleRing/${diamond._id}`,
                                      state: { ringData },
                                    }}
                                  >
